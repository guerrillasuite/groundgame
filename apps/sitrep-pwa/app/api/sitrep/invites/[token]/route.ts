...
// GET /api/sitrep/invites/[token] — public, returns invite details
...
// POST /api/sitrep/invites/[token]  { action: "accept" | "decline" }
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  const user = await getCrmUser().catch(() => null);
  if (!user) return NextResponse.json({ error: "sign_in_required" }, { status: 401 });

  await Promise.all([
    sb().from("squad_invites").update({ status: "accepted", accepted_at: now, user_id: user.userId }).eq("id", invite.id),
