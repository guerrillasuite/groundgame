/**
 * Shared calendar visibility filter logic for CRM SitRep.
 * Rule table:
 ...
 *
 * CRM items may have _source_tenant_id set for cross-tenant shared view items.
 */
...
export type SharedViewData = {
  ...
  owner_name:    string;
  // sources from the owner's calendar type
  type_sources?: CalendarSource[];
};

type ItemLike = {
  tenant_id?:           string;
  _source_tenant_id?:   string;  // set on cross-tenant items
  visibility?:          string;
  created_by?:          string;
  status?:              string | null;
  item_type?:           string;
...
function effectiveTenantId(item: ItemLike): string {
  return item._source_tenant_id ?? item.tenant_id ?? "";
}
