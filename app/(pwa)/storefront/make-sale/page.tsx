...
export default function MakeSalePage() {
  ...
  useEffect(() => {
    fetch("/api/survey/intake?channel=make_sale")
      .then((r) => r.json())
      .then((data) => {
  ...
  if (!survey) {
    return (
      <section className="stack" style={{ padding: 32 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Make Sale</h1>
        <p style={{ opacity: 0.6, marginTop: 8 }}>
          No Make Sale form configured. Go to{" "}
          <a href="/crm/survey" style={{ color: "var(--gg-primary, #2563eb)" }}>
            Survey Builder
          </a>{" "}
          and set a survey's default intake form to <strong>Make Sale</strong>.
        </p>
      </section>
    );
  }
