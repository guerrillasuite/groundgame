...
const SEARCH_ENDPOINTS: Record<EntityType, string> = {
  people:     "/api/crm/people/search",
  households: "/api/crm/households/search",
  locations:  "/api/crm/locations/search",
  companies:  "/api/crm/companies/search",
};

// ── Filter operator configs ───────────────────────────────────────────────────
    ...
    case "households":
      return {
        ...
        address: item.address
          ? [item.address, item.city, item.state, item.postal_code].filter(Boolean).join(", ")
          : [item.address_line1, item.city, item.state, item.postal_code].filter(Boolean).join(", "),
      };
    case "locations":
      return {
        id: item.id,
...
// ── Edit field definitions per entity ────────────────────────────────────────

function getEditFields(target: EntityType, contactTypes: ContactType[]): FieldDef[] {
  const ctOpts = [
    { value: "", label: "— none —" },
    ...contactTypes.map((ct) => ({ value: ct.key, label: ct.label })),
  ];
  switch (target) {
    case "people":
      return [
        {
          ...
          options: contactTypes.map((ct) => ({ value: ct.key, label: ct.label })),
        },
        {
          key: "contact_type",
          label: "Contact Type (legacy single)",
          inputType: "select",
          options: ctOpts,
        },
...
let _filterId = 0;
const freshFilterRow = (): FilterRow => ({
  id: ++_filterId, field: "", op: "contains", value: "", data_type: "text",
});
...
export default function BulkEditPanel() {
  ...
  const [filterRows, setFilterRows]       = useState<FilterRow[]>([freshFilterRow()]);
  ...
  useEffect(() => {
    fetch("/api/crm/settings/contact-types")
      .then((r) => r.ok ? r.json() : [])
      .then((data) => setContactTypes(Array.isArray(data) ? data : []))
      .catch(() => {});
  }, []);

  // ── Reset state on entity tab change ─────────────────────────────────────
  useEffect(() => {
    setQuery("");
    setSearchResults([]);
    setSearchTotal(0);
    setSearchSearched(false);
    setFilterRows([freshFilterRow()]);
    setFilterResults([]);
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    try {
      const endpoint = SEARCH_ENDPOINTS[target];
  ...
  async function runFilter() {
    ...
    try {
      ...
      setFilterResults(
        (Array.isArray(data) ? data : []).map((item: any) => normalizeRow(target, item))
      );
    } catch {
      setFilterResults([]);
    } finally {
  ...
  function getEdit(key: string, inputType: string): FieldEdit {
    return fieldEdits[key] ?? {
      enabled: false,
      value: inputType === "multiselect" ? [] : "",
      mode: "replace",
    };
  }
  function setFieldEnabled(key: string, enabled: boolean, inputType: string) {
    setFieldEdits((prev) => ({
      ...prev,
      [key]: { ...(prev[key] ?? { value: inputType === "multiselect" ? [] : "", mode: "replace" as const }), enabled },
    }));
  }
  function setFieldValue(key: string, value: any, inputType: string) {
    setFieldEdits((prev) => ({
      ...prev,
      [key]: { ...(prev[key] ?? { enabled: false, mode: "replace" as const, value: inputType === "multiselect" ? [] : "" }), value },
    }));
  }
  function setFieldMode(key: string, mode: "replace" | "append") {
    setFieldEdits((prev) => ({
      ...prev,
      [key]: { ...(prev[key] ?? { enabled: false, value: "" }), mode },
    }));
  }
  function toggleChip(key: string, chipValue: string) {
    const current = (fieldEdits[key]?.value ?? []) as string[];
    const next    = current.includes(chipValue)
      ? current.filter((v) => v !== chipValue)
      : [...current, chipValue];
    setFieldValue(key, next, "multiselect");
  }
    ...
    return (
      <section className="stack" style={{ maxWidth: 480 }}>
        ...
        <div style={{
          ...
          <p style={{ marginTop: 8, fontSize: 14, color: "var(--gg-text-dim, #6b7280)" }}>
            This page requires admin permissions. Please log in as an admin user.
          </p>
        </div>
      </section>
    );
  ...
  return (
    <section className="stack">
      ...
      {/* Entity tabs */}
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid var(--gg-border, #e5e7eb)" }}>
        {ENTITY_TABS.map((tab) => (
          <button
            key={tab.value}
            ...
            style={{
              padding: "9px 18px",
              fontSize: 14,
              fontWeight: target === tab.value ? 600 : 400,
              background: "none",
              border: "none",
              borderBottom: target === tab.value
                ? "2px solid var(--gg-primary, #2563eb)"
                : "2px solid transparent",
              color: target === tab.value
      ...
      {mode === "search" && (
        <div>
          ...
          />
          {searchLoading && (
            <p style={{ margin: "8px 0 0", fontSize: 13, color: "var(--gg-text-dim, #9ca3af)" }}>Searching…</p>
          )}
        </div>
      )}
        ...
        <div style={{
          ...
          {filterRows.map((row, i) => {
            ...
            return (
              <div key={row.id} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: "var(--gg-text-dim)", minWidth: 40, textAlign: "right" }}>
                  {i === 0 ? "WHERE" : "AND"}
                </span>
                <select
                  value={row.field}
                ...
                <select
                  value={row.op}
                  onChange={(e) => updateFilterRow(row.id, { op: e.target.value as FilterOp })}
                  style={selectStyle}
                >
                ...
                {filterRows.length > 1 && (
                  <button
                    ...
                    style={{ padding: "4px 8px", fontSize: 13, border: "none", background: "none", color: "var(--gg-text-dim, #9ca3af)", cursor: "pointer" }}
                  >
                    ×
                  </button>
                )}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button
              ...
              style={{ padding: "6px 14px", fontSize: 13, border: "1px solid var(--gg-border, #e5e7eb)", borderRadius: 5, background: "var(--gg-bg, #fff)", cursor: "pointer" }}
            >
              + Add Filter
            </button>
            <button
              onClick={runFilter}
              disabled={filterLoading || filterRows.every((r) => !r.field)}
              style={{
                padding: "6px 18px",
                fontSize: 13,
                fontWeight: 600,
                ...
                color: "#fff",
                cursor: filterLoading ? "default" : "pointer",
        ...
        <>
          ...
          {currentResults.length > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", padding: "8px 0" }}>
              ...
              {selected.size > 0 && (
                <button
                  onClick={deselectAll}
                  style={{ padding: "3px 10px", fontSize: 12, border: "1px solid var(--gg-border, #e5e7eb)", borderRadius: 4, background: "none", cursor: "pointer" }}
                >
                  Deselect all
                </button>
              )}
          ...
          {currentResults.length === 0 ? (
            <div style={{
              ...
              No records found matching your {mode === "search" ? "search" : "filters"}.
            </div>
          ) : (
                  ...
                  {pageRows.map((row) => (
                    <tr
                      ...
                      style={{
                        ...
                        background: selected.has(row.id) ? "var(--gg-primary-10, #eff6ff)" : undefined,
                        cursor: "pointer",
                      }}
                    >
          ...
          {currentResults.length > perPage && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button
                ...
                style={{ padding: "4px 10px", fontSize: 12, border: "1px solid var(--gg-border)", borderRadius: 4, background: "var(--gg-bg, #fff)", cursor: page === 0 ? "default" : "pointer" }}
              >‹ Prev</button>
              <button
                disabled={(page + 1) * perPage >= currentResults.length}
                onClick={() => setPage((p) => p + 1)}
                style={{ padding: "4px 10px", fontSize: 12, border: "1px solid var(--gg-border)", borderRadius: 4, background: "var(--gg-bg, #fff)", cursor: (page + 1) * perPage >= currentResults.length ? "default" : "pointer" }}
              >Next ›</button>
            </div>
          )}
        </>
      ...
      {selected.size > 0 && (
        <div style={{
          border: "2px solid var(--gg-primary, #2563eb)",
          borderRadius: 8,
          padding: "20px 24px",
          background: "var(--gg-bg, #fff)",
        }}>
                ...
                <div key={fd.key}>
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                    <input
                      ...
                      onChange={(e) => setFieldEnabled(fd.key, e.target.checked, fd.inputType)}
                    />
                    <span style={{ fontWeight: 600, fontSize: 14 }}>{fd.label}</span>
                  </label>

                  {edit.enabled && (
                    <div style={{ marginLeft: 24, marginTop: 8, display: "flex", flexDirection: "column", gap: 8 }}>
                      {/* Replace / Append toggle for notes */}
                      {fd.hasMode && (
                          ...
                          {(["replace", "append"] as const).map((m) => (
                            <label key={m} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 13, cursor: "pointer" }}>
                              <input
                                ...
                                name={`mode_${fd.key}`}
                                value={m}
                                checked={(edit.mode ?? "replace") === m}
                                onChange={() => setFieldMode(fd.key, m)}
                              />
                              {m === "replace" ? "Replace existing" : "Append to existing"}
                            </label>
                          ))}
                        </div>
                      )}
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {(fd.options ?? []).length === 0 && (
                            <span style={{ fontSize: 13, color: "var(--gg-text-dim, #9ca3af)" }}>
                              No contact types configured yet.
                            </span>
                          )}
                          {(fd.options ?? []).map((opt) => {
                            const isActive = ((edit.value ?? []) as string[]).includes(opt.value);
                            return (
                              <button
                                key={opt.value}
                                onClick={() => toggleChip(fd.key, opt.value)}
                                style={{
                                  padding: "5px 14px",
                                  borderRadius: 20,
                                  fontSize: 13,
                                  ...
                                  border: "1px solid",
                                  borderColor: isActive ? "var(--gg-primary, #2563eb)" : "var(--gg-border, #e5e7eb)",
                                  background: isActive ? "var(--gg-primary, #2563eb)" : "transparent",
                                  color: isActive ? "#fff" : "var(--gg-text, #374151)",
                                  cursor: "pointer",
                                }}
                              >
                                {opt.label}
                      ...
                      {fd.inputType === "select" && (
                        <select
                          value={edit.value ?? ""}
                          onChange={(e) => setFieldValue(fd.key, e.target.value, fd.inputType)}
                      ...
                      {fd.inputType === "textarea" && (
                        <textarea
                          ...
                          onChange={(e) => setFieldValue(fd.key, e.target.value, fd.inputType)}
                          rows={3}
                          placeholder={edit.mode === "append" ? "Text to append…" : "New value (replaces existing)…"}
                          style={{
                            padding: "6px 10px",
                            border: "1px solid var(--gg-border, #e5e7eb)",
                            borderRadius: 5,
          ...
          <div style={{ marginTop: 20, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            {!confirmOpen ? (
              <button
                disabled={!hasValidEdits || applying}
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: "#92400e" }}>
                  Update {selected.size} {target} records?
                </span>
                <button
                  onClick={applyEdits}
                  disabled={applying}
