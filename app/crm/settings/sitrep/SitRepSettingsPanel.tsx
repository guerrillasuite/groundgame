...
export default function SitRepSettingsPanel({ isDirector = true, currentUserId = "" }: { isDirector?: boolean; currentUserId?: string }) {
  ...
  type PendingSquadInvite = { id: string; squadId: string; squadName: string; squadColor: string; inviterName: string; token: string };
  const [pendingSquadInvites,   setPendingSquadInvites]   = useState<PendingSquadInvite[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/user/squad-invites")
      .then((r) => r.ok ? r.json() : [])
      .then((data) => setPendingSquadInvites(Array.isArray(data) ? data : []))
      .catch(() => {})
  ...
  async function respondToSquadInvite(token: string, action: "accept" | "decline") {
    ...
    try {
      ...
      if (res.ok) {
        setPendingSquadInvites((p) => p.filter((i) => i.token !== token));
        if (action === "accept") {
          fetch("/api/crm/sitrep/squads")
            .then((r) => r.ok ? r.json() : [])
            .then((data) => setSquads(Array.isArray(data) ? data : []))
    ...
    <>
      <div className="stack" style={{ maxWidth: 680 }}>
        ...
        <div style={{
          ...
          {!pendingInvitesLoading && pendingSquadInvites.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: S.dim, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 10 }}>
                Pending Invitations
              </div>
              <div style={{ display: "grid", gap: 6 }}>
                {pendingSquadInvites.map((inv) => {
                  const dot  = getFamilyByKey(inv.squadColor)?.shades[2] ?? "#818cf8";
                  ...
                  return (
                    <div key={inv.id} style={{
                      ...
                      <button
                        type="button"
                        onClick={() => respondToSquadInvite(inv.token, "accept")}
                        disabled={busy}
                        className="btn"
                        style={{ padding: "5px 12px", fontSize: 12, borderRadius: 7, opacity: busy ? 0.5 : 1, flexShrink: 0 }}
                      >{busy ? "…" : "Accept"}</button>
                      <button
                        type="button"
                        onClick={() => respondToSquadInvite(inv.token, "decline")}
                        disabled={busy}
                        style={{
                              ...
                              <div>
                                ...
                                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                                  <input
                                    ...
                                    style={{
                                      flex: 1, minWidth: 160, padding: "6px 10px", borderRadius: 7, fontSize: 12,
                                  ...
                                  <input
                                    ...
                                    style={{
                                      flex: 1, minWidth: 130, padding: "6px 10px", borderRadius: 7, fontSize: 12,
                                      background: S.card, border: `1px solid ${S.border}`, color: S.text, outline: "none",
                                    }}
                                  />
                                </div>
                                <div style={{ display: "flex", gap: 8 }}>
                                  ...
                                  <button
                                    ...
                                    className="btn"
                                    style={{ padding: "6px 14px", fontSize: 12, borderRadius: 7, opacity: inviteSending === squad.id ? 0.6 : 1, flexShrink: 0 }}
